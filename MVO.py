...
def MVO(objf, lb, ub, dim, N, Max_time):
    ...
    while Time < Max_time + 1:
        ...
        Time = Time + 1
    timerEnd = time.time()
    x = numpy.arange(0, Max_time, dtype=int) + 1
    timerEnd = time.time()
    print('Completed in', (timerEnd - timerStart))
    plt.plot(x, convergence, 'o-')
    plt.xlabel("Iterations")
    plt.ylabel("Fitness")
    plt.title(
        f"Convergence_curve for MVO for parameter including population "
        f"{N}, \niteration {Max_time},and max fitness is:{min(convergence)}")
    plt.show()

    opts ={"p":Best_universe,'c':min(convergence)}
